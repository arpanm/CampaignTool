...
import com.arpan.campaigntool.config.KafkaProperties;
...
class CampaignToolKafkaResourceIT {
    ...
    void setup() {
        ...
        CampaignToolKafkaResource kafkaResource = new CampaignToolKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/campaign-tool-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/campaign-tool-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
