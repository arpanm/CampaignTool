...
import com.reliance.jpl.repository.CallRepository;
import com.reliance.jpl.service.CallService;
...
/**
 * REST controller for managing {@link com.reliance.jpl.domain.Call}.
 */
@RestController
@RequestMapping("/api")
public class CallResource {
    ...
    public ResponseEntity<CallDTO> updateCall(@PathVariable(value = "id", required = false) final Long id, @RequestBody CallDTO callDTO)
        ...
        CallDTO result = callService.update(callDTO);
        return ResponseEntity
            .ok()
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/calls/{id}", consumes = { "application/json", "application/merge-patch+json" })
     ...
     */
    @GetMapping("/calls")
    public ResponseEntity<List<CallDTO>> getAllCalls(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
